...
* 1~2계층에서는 프레임이라 함
* 3계층 이상에서는 패킷이라 함
* IP헤더 ![IP헤더](https://mblogthumb-phinf.pstatic.net/20150330_184/sujunghan726_1427695807924ArlQA_PNG/2015-03-30_15%3B09%3B50.PNG?type=w2)


### 유의적 버전(SemVer, Semantic Versioning)
* 요약
    * 버전을 주.부.수(x.y.z, 메이저.마이너.패치) 숫자로 하고
        * "주(主)버전" 증가 : 기존 버전과 호환되지 않게 API 변경
        * "부(部)버전" 증가 : 기존 버전과 호화되면서 새로운 기능 추가
        * "수(修)버전" 증가 : 기존 버전과 호화되면서 버그를 수정한 경우
* [링크](https://semver.org/lang/ko/)


### JIT vs AOT
* JIT(Just In Time)
    * 런타임 때 컴파일을 포함하여 코드를 실행하는 방법
    * 런타임전이 아니라 필요할 때 코드를 컴파일
    * 모든 코드를 처음에 기계어로 변환하는 것이 아닌 필요한것들만 변환
    * 장점
        * 필요할 때에만 사용하기 때문에 CPU의 부담 감소
* AOT(Ahead Of Time)
    * 미리 코드를 시스템에 맞는 기계어로 변환
    * 장점
        * 실행속도 빠름
    * 단점
        * 빌드시 JIT보다 시간이 더 걸리고 파일이 클 수 있음

* 바이트코드(bytecode) : 인터프리터에 의해 바로 실행 될 수 있는 소스 코드와 기계어(marchine code) 중간의 코드
* 기계어(machine code) : 컴퓨터의 물리적인 프로세서에서 바로 동작할 수 있는 이진 코드
...
### Observer Pattern
* 객체의 상태 변화를 관찰하는 옵저버들의 목록을 객체에 등록하여 상태 변화가 있을 때마다 메서드등을 통해 객체가 직접 목록의 각 옵저버에게 통지하도록 하는 디자인 패턴